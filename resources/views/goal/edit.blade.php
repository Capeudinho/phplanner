    ...
    <form id="Form" action="{{ route('goal.update', $goal->id) }}" method="POST">
        ...
        <div class="flex mb-6">
            <div style="display: inline-block; width: 100%;">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $goal->event->title) }}" class="w-full px-4 py-2 border border-gray-300 rounded-md @error('title') border-red-500 @enderror" placeholder="Título" style="margin-top: 4px">
                @error('title')
        ...
        <div class="flex mb-6">
            <div style="display: inline-block; width: 100%;">
                ...
                <textarea name="description" id="description" class="w-full px-4 py-2 border border-gray-300 rounded-md @error('description') border-red-500 @enderror" placeholder="Descrição" rows="5" style="margin-top: 4px; resize: none">{{ old('description', $goal->event->description) }}</textarea>
                @error('description')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="flex mb-6">
			<div style="display: inline-block; width: 49%; margin-right: 2%;">
                ...
                <input type="datetime-local" name="start" id="start" class="w-full px-4 py-2 border border-gray-300 rounded-md @error('start') border-red-500 @enderror" value="{{ old('start', $goal->event->start) }}" style="margin-top: 4px">
                @error('start')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div style="display: inline-block; width: 49%;">
                ...
                >
                    <option value="week" {{ (old('duration', $goal->duration) == 'week') ? 'selected' : '' }}>1 semana</option>
                    <option value="month" {{ (old('duration', $goal->duration) == 'month') ? 'selected' : '' }}>1 mês</option>
                    <option value="year" {{ (old('duration', $goal->duration) == 'year') ? 'selected' : '' }}>1 ano</option>
                </select>
                @error('duration')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            ...
            <div style="display: inline-block; width: 49%;">
                ...
                >
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" style="color: {{ $category->color }}" color="{{ $category->color }}" {{ (old('category_id', $goal->event->category_id) == $category->id) ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </select>
                @error('category_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
